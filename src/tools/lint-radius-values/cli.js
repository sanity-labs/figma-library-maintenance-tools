...
const HELP_TEXT = `
Usage: figma-lint-radius [options]

Detects nodes with border radius values not bound to a radius variable.
...
Note: This tool requires variable data for the Radius collection. When using
--stdin, the JSON payload must include a "variablesData" key alongside "fileData".
`;
...
async function main() {
  try {
    ...
    const report = await lintRadiusValues(config);
